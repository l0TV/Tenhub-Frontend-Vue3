    ...
    <div class="auth-wrap page-width">
      ...
      <section class="auth-card" :aria-label="authMode === 'login' ? '登录' : '注册'">
        <div class="auth-card-head">
          <div>
            <h2>{{ authMode === 'login' ? '欢迎登录' : '创建拾汇账号' }}</h2>
            <p>{{ authMode === 'login' ? '登录后享受更多会员权益' : '注册即可领取新人专享礼包' }}</p>
          </div>
        ...
        <div v-if="wechatQrUrl" class="wechat-qr-panel">
          <img :src="wechatQrImage" alt="微信登录二维码" />
          <div>
            <b>微信扫码登录</b>
            <p>请使用微信扫一扫，授权后即可登录或自动注册。</p>
            <a :href="wechatQrUrl" target="_blank" rel="noreferrer">在微信中打开授权链接 ↗</a>
          </div>
        </div>
